...
| Interface de `Repository` | **Fica junto da implementação** em `Modules/<Nome>/Repositories/`, não em `Contracts` — `Contracts` é reservado ao que atravessa a fronteira do módulo (`I<NomeModulo>`, `IntegrationEvents`, `Dtos`); `Repository` nunca é injetado por outro módulo, sua visibilidade `public` é só consequência do C# (`ARCHITECTURE-RULES.md` seção 5.1) (2026-07-17) |
