...
| Interface de `Repository` | **Continua dentro de `Modules/<Nome>/Repositories/`, não em `Shared/Contracts`** — esse `Contracts` é reservado ao que atravessa a fronteira do módulo (`I<NomeModulo>`, `IntegrationEvents`, `Dtos`); `Repository` nunca é injetado por outro módulo, sua visibilidade `public` é só consequência do C# (`ARCHITECTURE-RULES.md` seção 5.1). **Decisão final:** a interface fica em `Repositories/Contracts/I<Nome>Repository.cs` (pasta local, um terceiro uso do nome "Contracts" só como convenção de "aqui moram interfaces", sem relação com `Shared/Contracts`); a implementação concreta fica na raiz de `Repositories/` (`REPOSITORIES/RULES.md` seção 2) (2026-07-17, tentativa intermediária com `Interface/`+`Implementation/` em 2026-07-19, substituída por `Contracts/`+raiz no mesmo dia após revisão) |
