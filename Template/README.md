...
| Roteamento Command → Handler | **Resolução manual via DI** (sem biblioteca mediator) |
...
| Handler — granularidade | **Um `Handler` por Aggregate Root/recurso** (tipicamente um por `Controller`), implementando `Shared.Kernel.IHandler<TCommand,TResult>` uma vez por `Command` aceito (mutação ou leitura) — não um `Handler` por `Command` individual. Segundo `Handler` só quando o `Controller` expõe um substantivo distinto sob outro subcaminho (critério "banana vs. tomate", ex: `GeneroHandler` + `AnimeHandler` sob `CatalogoController`) (`HANDLER/RULES.md` seções 3-4) (2026-07-19) |
...
| Reforço — exemplos ❌/✅ em todo `RULES.md` de módulo | Prosa sozinha ("não pode conter X") se mostrou insuficiente para um agente isolado, que só tem acesso ao rulebook via git, seguir a regra corretamente. Todo `RULES.md` de `03-MODULES/*` (mais `SECURITY/RULES.md`) ganhou pelo menos um par de código `❌ Errado`/`✅ Correto` ao lado do anti-padrão mais arriscado daquele nó: `Repository<T>` genérico (`REPOSITORIES`), `Entity` anêmica (`ENTITIES`), `Dto`/fachada vazando `Entity` (`CONTRACTS`), `Service` chamando `Handler` (`SERVICES`), `Scoped` injetado direto num `Consumer` `Singleton` (`CONSUMERS`), string literal em vez de `Dictionary` (`DICTIONARY`), `Command` mutável (`COMMANDS`), `ModuleInstaller` malformado (`03-MODULES/RULES.md`), `Handler` devolvendo `Entity`/engolindo exceção (`HANDLER`), e a confusão de três vias `Result.Failure`/`DomainException`/`AppException` (`SECURITY`) (2026-07-19) |
| Reforço — exemplos ❌/✅ em Host e Infrastructure | Mesma lacuna da linha acima, estendida às camadas de composição/infra: `01-HOST/RULES.md` ganhou o par módulo listado por nome vs. descoberta por assembly, e seção lendo config de outro módulo; `DATABASE/RULES.md` ganhou `Repository` abrindo transação própria (bypass de `IUnitOfWork`) e `SELECT *`/SQL concatenado; `MESSAGING/RULES.md` ganhou `Handler` publicando direto no RabbitMQ (bypass do Outbox) e `Consumer` sem checagem de idempotência; `CACHE/RULES.md` ganhou módulo lendo chave de prefixo alheio (2026-07-19) |
...
| Gap de Integration test (RabbitMQ/Redis) — fechado | A pedido explícito do dev, implementado no `AnimeList` real: `Testcontainers.RabbitMq`/`Testcontainers.Redis` `4.13.0` adicionados a `Test/Watchlist/Integration` e `Test/Match/Integration` (registrados em `LIBRARIES.md` §2). Três testes novos: `RabbitMqOutboxPublishingTests` (prova que `OutboxPublisherService` publica de fato num broker real), `ProgressoAtualizadoConsumerTests` (round-trip completo + teste de idempotência com o mesmo evento entregue duas vezes), `RedisCacheServiceTests` (cache-aside + TTL expirando de verdade). `dotnet build` limpo (0 avisos, 0 erros) confirma a API dos pacotes está correta; execução real contra os containers **não foi validada neste ambiente** (Docker indisponível) — pendente rodar `dotnet test` com Docker Desktop ativo. Detalhado em `REFERENCE-IMPLEMENTATION.md` seção 3 (2026-07-19) |
| Nomenclatura `Command`/`Dto`/`Request`/`Entity` — escopo e sufixo único | **`Command`** (não mais `Command`/`Query` separados): sufixo único, mutação e leitura, verbo em inglês (`Create`/`Update`/`Get`/`Delete` ou verbo de domínio) identifica a ação — ex: `GetPedidoByIdCommand`, nunca `ObterPedidoPorIdQuery` (`COMMANDS/RULES.md` §4). **`Request`** — novo, formalizado em `Contracts/Requests/`: só parâmetro de ação do `Controller`, nunca cruza pro `Handler`. **`Dto`** — permanece o retorno do `Handler`/corpo de resposta HTTP; "Response" é só o nome do papel, não existe uma terceira classe (`CONTRACTS/RULES.md` §1.2). **`Entity`** — nome do substantivo puro, sem sufixo "Entity" na classe. Nomenclatura sempre PascalCase (`Dto`/`Request`, nunca `DTO`/`REQUEST`) — SonarQube (`S101`) rejeita sufixo todo maiúsculo; exceção documentada usa `[SuppressMessage]`/`// NOSONAR`. **Decisão aplicada só no Template** — o `AnimeList` real continua com a nomenclatura anterior (`CriarPedidoCommand`/`ObterPedidoPorIdQuery`), documentada como versão prévia válida em `REFERENCE-IMPLEMENTATION.md`, não como erro a corrigir (2026-07-19) |
