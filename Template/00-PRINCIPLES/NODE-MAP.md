...
| `Modules/<N>/Controller/` | Módulo | Superfície HTTP — 1 Controller por Aggregate Root/recurso | `[Authorize]`/`[AllowAnonymous]`, mapeamento request→Command, injeção do Handler | Regra de negócio, acesso a `Repository` direto | pública (exigido ASP.NET Core) | `03-MODULES/CONTROLLER/RULES.md` |
