...
| `Modules/<N>/Handler/` | Módulo | Orquestração — 1 Handler por recurso, implementa `IHandler<TCmd,TResult>` por Command aceito (mutação ou leitura) | Chamada a `Repository`/`Service`, `IUnitOfWork`, publicação de evento | Acesso a `IDbConnection` cru, captura de exceção de infra pra virar HTTP | pública | `03-MODULES/HANDLER/RULES.md` |
| `Modules/<N>/Contracts/Requests/` | Módulo | Forma de **entrada** HTTP — só parâmetro de ação do Controller | Records imutáveis, sem lógica | Uso fora do Controller, retorno de qualquer método | pública (exigido ASP.NET Core) | `03-MODULES/CONTRACTS/RULES.md` §1.2 |
| `Modules/<N>/Contracts/Dtos/` | Módulo | Forma de **saída** HTTP (Response) — retorno do Handler | Records/classes imutáveis, sem lógica | Referência a `Entity` | pública | `03-MODULES/CONTRACTS/RULES.md` §1.2 |
...
| `Modules/<N>/Commands/` | Módulo | Sufixo único `Command` — mutação e leitura, verbo em inglês (Create/Update/Get/Delete) distingue a ação | Records imutáveis de entrada do `Handler` | Lógica, acesso a dado, sufixo `Query`, verbo em português | pública | `03-MODULES/COMMANDS/RULES.md` §4, `03-MODULES/RULES.md` §7 |
...
| 1 `Handler` por Aggregate Root, não por Command | Contar classes em `Handler/` vs. contar `Command` em `Commands/` | Nº de Handlers ≤ nº de Controllers; cada Handler implementa `IHandler<>` 1x por Command que atende |
| Nomenclatura Command/Dto/Request consistente | `grep -rn "Query\b" Modules/*/Commands/*.cs` e `grep -rniE "DTO\b\|REQUEST\b\|ENTITY\b" --include=*.cs .` | Zero ocorrências de sufixo `Query`; zero sufixo todo maiúsculo sem `[SuppressMessage]`/`// NOSONAR` justificado (`CONTRACTS/RULES.md` §1.2) |
